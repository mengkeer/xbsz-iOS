...
- (void)updateUIWithIndex:(NSInteger)index option:(NSString *)option isSingle:(BOOL)isSingle{
    NSString *symbolText = [FMDBUtil indexConvertToSymbol:index];
...
- (void)showMutiPracticeResult:(NSInteger)index
    ...
    if(isRight){
        ...
        NSString *str = [NSString stringWithFormat:@"%ld",index];
        if([selectedIndexs containsString:str] && [FMDBUtil isSingleRightAnswer:index answer:answer]){
            [self updateOptionUI:QuestionStatusMutiRightWrong];
        }else if([FMDBUtil isSingleRightAnswer:index answer:answer] && ![selectedIndexs containsString:str]){
            [self updateOptionUI:QuestionStatusMutiRight];
        }else if(![FMDBUtil isSingleRightAnswer:index answer:answer] && [selectedIndexs containsString:str]){
